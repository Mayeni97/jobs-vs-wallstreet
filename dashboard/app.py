...
for k in REQ:
    if not os.getenv(k):
        st.error(f"Missing .env variable: {k}")
        st.stop()
...
ENGINE_URL = f"postgresql+psycopg2://{PGUSER}:{PGPASSWORD}@{PGHOST}:{PGPORT}/{PGDATABASE}?sslmode=require"
...
st.caption("U.S. unemployment & sector employment vs S&P 500 and sector ETFs — monthly aligned")

# ---------------- Universe autodetect ----------------
tickers = q("select distinct ticker from equities_monthly order by 1;")["ticker"].tolist()
if not tickers:
    st.error("No equities in DB. Load data first.")
    st.stop()
...
# Overall monthly overlap across tables
meta = q("""
    select 
...
with st.sidebar:
    ...
    start = st.date_input("Start month", value=start_default, min_value=start_default, max_value=end_default, key="start_date")
    end   = st.date_input("End month",   value=end_default,   min_value=start_default, max_value=end_default, key="end_date")
...
equities = q("""
    select date_trunc('month', period_date)::date as ym,
           ticker, 
           max(adj_close) as adj_close,          -- end-of-month
...
def nonempty(df, name):
    if df.empty:
        st.warning(f"No rows for {name} in the selected window.")
        return False
    return True
...
# ---------------- KPIs ----------------
def pct_window(series: pd.Series, months: int) -> float | None:
    if len(series) < months or months <= 0:
        return None
    # cumulative (1+r) product over last N months
...
if ok_u and ok_e:
    # Unemployment level + MoM / YoY deltas (percentage points)
    ...
    # Sector ETF momentum + correlation with jobs Δ% (m/m)
    ...
    corr_df = pd.merge(
        etf[["ym","monthly_return"]], sj[["ym","emp_pct_change"]],
        on="ym", how="inner"
    ).dropna()
    ...
    kpi[2].metric(
        ...
        if (r3_e is not None and r12_e is not None) else "—"
    )

# ---------------- Plots ----------------
col1, col2 = st.columns(2)

# A) Unemployment vs S&P (indexed)
if ok_u and ok_e:
    ...
    if not spx.empty:
        ...
        if not df_a.empty:
            fig = px.line(
                df_a, x="ym", y=["unemployment_rate","spx_indexed"],
        ...
        col1.info(f"No {SPX} rows in selected window.")

# B) Sector jobs vs ETF (indexed)
if ok_s and ok_e:
    ...
    if not etf.empty:
        ...
        if not df_b.empty:
            fig2 = px.line(
                df_b, x="ym", y=["employment_thousands","etf_indexed"],
                labels={"value":"Value","ym":"Month","variable":"Series"},
        ...
        col2.info(f"No {sector_etf} rows in selected window.")

# C) Correlation snapshot table (monthly)
...
st.caption("All series aligned by month (date_trunc('month')) to avoid day mismatches. ‘Momentum’ is 1m/3m/12m total return.")